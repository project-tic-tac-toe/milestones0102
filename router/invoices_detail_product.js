...
router.post("", async function(req, res, next) {
  //id_employee
  if (!req.body.id_employee || !req.body.id_invoice || !req.body.id_product) {
    res.send({
      code: 0,
  ...
  let prize_product =
    id_product === null
      ? 0
      : await getPrizeOfProduct(id_product).then(res => res);
...
router.put("/:id", async function(req, res, next) {
  ...
  const dataOld = await getInvoicesDetailProduct(idFind).then(res => res);
  if (dataOld === null) return;
...
router.del("/:id", async function(req, res, next) {
  ...
  const item = await getInvoicesDetailProduct(idFind).then(res => res);
  knex("invoices_detail_product")
    .where({ id: idFind })
    .del()
    .then(result => {
